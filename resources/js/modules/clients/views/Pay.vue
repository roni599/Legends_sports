  ...
  <div class="content-card">
    <div class="content-header">
      <div class="d-flex justify-content-between align-items-center w-100">
        <h3 class="fs-5 m-0 text-light">Client Due Pay</h3>
    ...
    <div class="p-4" v-else-if="client">
      ...
      <div class="mb-4 p-3 bg-dark rounded border border-secondary">
        <h5 class="text-light mb-0">Current Balance: 
          <span :class="client.total_due < 0 ? 'text-success' : 'text-danger'">
            ৳{{ Math.abs(client.total_due).toFixed(2) }} 
            {{ client.total_due < 0 ? '(Advance)' : '(Due)' }}
          </span>
        </h5>
      </div>
          ...
          <div class="row">
            ...
            <div class="col-sm-9">
              <button class="btn btn-primary w-100 py-2 fw-bold" @click="submitPayment" :disabled="submitting || paymentForm.amount <= 0">
                ...
                <i v-else class="bi bi-check-circle me-1"></i> Confirm Payment
              </button>
            </div>
          </div>
        </div>
...
const handleAmountInput = () => {
  ...
  const maxPayable = client.value.total_due < 0 ? Math.abs(client.value.total_due) : 0;
  let val = Number(paymentForm.value.amount);
  if (val > maxPayable) {
    paymentForm.value.amount = maxPayable;
    Swal.fire({ toast: true, position: 'top-end', icon: 'warning', title: `Maximum payable amount is ${maxPayable}`, showConfirmButton: false, timer: 3000 });
  }
};
...
const submitPayment = async () => {
  ...
  try {
    await axios.post(`/api/clients/${clientId}/pay-out`, {

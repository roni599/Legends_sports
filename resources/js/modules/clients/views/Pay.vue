  ...
  <div class="content-card">
    <div class="content-header">
      <div class="d-flex justify-content-between align-items-center w-100">
        <h3 class="fs-5 m-0 text-light">Pay Out Refund</h3>
        ...
        <table class="table table-dark table-striped table-hover table-bordered align-middle">
          <thead style="background-color: #444;">
            <tr>
              ...
              <th class="ps-0">Transaction / Invoice</th>
              <th>Type</th>
              <th>Date</th>
              <th>Paid Amount (৳)</th>
              <th>Refundable (৳)</th>
          ...
          <tbody>
            <tr v-if="loadingInvoices">
              <td colspan="7" class="text-center py-4">Loading refundable items...</td>
            </tr>
            <tr v-else-if="invoices.length === 0">
              <td colspan="7" class="text-center py-4">No refundable bookings or invoices found.</td>
            </tr>
            <tr v-else v-for="invoice in invoices" :key="invoice.id">
              ...
              <td>{{ invoice.date }}</td>
              <td>{{ invoice.paid_amount || invoice.grand_total }}</td>
      ...
      <div class="row mt-4">
        ...
        <div class="col-md-6">
          ...
          <div class="row g-0 mb-3 align-items-center">
            <label class="col-sm-3 col-form-label text-light fw-bold text-end pe-2">Total Refundable</label>
            <div class="col-sm-9">
...
const fetchClientAndInvoices = async () => {
  try {
    ...
    const invRes = await axios.get(`/api/clients/${clientId}/refundable-invoices`);
...
const submitPayment = async () => {
  ...
  try {
    await axios.post(`/api/clients/${clientId}/refund-invoices`, {
    ...
    Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Refund processed successfully!', showConfirmButton: false, timer: 2000 });
    router.push('/clients');
  } catch (error) {

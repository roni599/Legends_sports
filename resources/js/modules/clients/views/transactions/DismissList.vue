  ...
  <div class="content-card">
    <div class="content-header">
      <div class="d-flex justify-content-between align-items-center w-100">
        <h3 class="fs-5 m-0 text-light">Dismiss List</h3>
        ...
        <table class="table table-dark table-striped table-hover table-bordered align-middle">
          ...
          <tbody>
            ...
            <tr v-else-if="transactions.length === 0">
              <td colspan="7" class="text-center py-4">No dismissals found.</td>
            </tr>
            <tr v-else v-for="(tx, index) in transactions" :key="tx.id">
              ...
              <td class="text-warning fw-bold">{{ tx.amount }}</td>
...
const fetchTransactions = async (page = 1) => {
  ...
  try {
    const res = await axios.get('/api/client-transactions', {
      params: { type: 'due_dismiss', page, search: search.value }
    });
...
const goToEditPage = (tx) => {
  router.push(`/clients/transactions/dismiss/${tx.id}/edit`);
};
...
const viewInvoice = (tx) => {
  if (tx.invoice_id) {
    ...
    Swal.fire({ toast: true, position: 'top-end', icon: 'info', title: 'Receipt viewing not available for this record', showConfirmButton: false, timer: 3000 });
  }
};
</script>

  ...
  <div class="content-card">
    <div class="content-header">
      <div class="d-flex justify-content-between align-items-center w-100">
        <h3 class="fs-5 m-0 text-light">Due Receive List</h3>
...
const fetchTransactions = async (page = 1) => {
  ...
  try {
    const res = await axios.get('/api/client-transactions', {
      params: { type: 'due_receive', page, search: search.value }
    });
...
const goToEditPage = (tx) => {
  router.push(`/clients/transactions/due-receive/${tx.id}/edit`);
};

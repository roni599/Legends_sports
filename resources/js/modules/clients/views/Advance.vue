<template>
  <div class="content-card">
    ...
    <div class="p-4" v-else-if="client">
      ...
      <!-- Invoices / Transactions Table for Refunds -->
      <div class="table-responsive mb-4">
        <h5 class="text-light mb-3">Refund Existing Advances</h5>
        <table class="table table-dark table-striped table-hover table-bordered align-middle">
          <thead style="background-color: #444;">
            <tr>
              ...
              <th>Paid Amount (৳)</th>
              <th>Refundable (৳)</th>
              <th style="width: 250px;">Refunding Amount (৳)</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="loadingInvoices">
              <td colspan="7" class="text-center py-4">Loading advance invoices...</td>
            </tr>
            <tr v-else-if="invoices.length === 0">
              <td colspan="7" class="text-center py-4">No advance invoices found.</td>
            </tr>
            <tr v-else v-for="invoice in invoices" :key="invoice.id">
              ...
              <td>{{ invoice.date }}</td>
              <td>{{ invoice.paid }}</td>
              <td class="text-success fw-bold">{{ invoice.due }}</td>
              <td>
                <input type="text" 
      ...
      <div class="row mt-4">
        ...
        <div class="col-md-6">
          ...
          <div class="row g-0 mb-3 align-items-center">
            <label class="col-sm-4 col-form-label text-light fw-bold text-end pe-2">Total Advance Available</label>
            <div class="col-sm-8">
              <div class="input-group">
                <span class="input-group-text bg-dark text-light border-secondary">৳</span>
                <input type="text" class="form-control modal-input text-success fw-bold fs-5" :value="grandTotalDue.toFixed(2)" readonly>
              </div>
            </div>
          </div>
          
          <div class="row g-0 mb-3 align-items-center">
            <label class="col-sm-4 col-form-label text-light text-end pe-2">Receiving Amount</label>
            <div class="col-sm-8">
              <div class="input-group">
                <span class="input-group-text bg-dark text-light border-secondary">৳</span>
                <input type="text" class="form-control modal-input fw-bold text-success fs-5" v-model.number="paymentForm.receiving_advance">
              </div>
            </div>
          </div>
          
          <div class="row g-0 mb-3 align-items-center">
            <label class="col-sm-4 col-form-label text-light text-end pe-2">Refunding Amount</label>
            <div class="col-sm-8">
              <div class="input-group">
                <span class="input-group-text bg-dark text-light border-secondary">৳</span>
                <input type="text" class="form-control modal-input fw-bold text-success fs-5" v-model="paymentForm.refund_advance" @input="handleGlobalAmountInput">
              </div>
            </div>
          </div>
          
          <div class="row g-0 mb-3 align-items-center">
            <label class="col-sm-4 col-form-label text-light text-end pe-2">Transaction Date</label>
            <div class="col-sm-8">
              <div class="input-group">
          ...
          <div class="row g-0 mb-4 align-items-center">
            <label class="col-sm-4 col-form-label text-light text-end pe-2">Payment Method</label>
            <div class="col-sm-8">
              <div class="input-group">
          ...
          <div class="row">
            ...
            <div class="col-sm-8">
              <button class="btn btn-primary w-100 py-2 fw-bold" @click="submitPayment" :disabled="submitting || (paymentForm.receiving_advance <= 0 && paymentForm.refund_advance <= 0)">
                <span v-if="submitting" class="spinner-border spinner-border-sm me-2"></span>
                <i v-else class="bi bi-check-circle me-1"></i> Confirm Advance / Refund
              </button>
            </div>
          </div>
        </div>
...
const fetchClientAndInvoices = async () => {
  try {
    ...
    const invRes = await axios.get(`/api/clients/${clientId}/advance-invoices`);
...
const toggleAll = () => {
  ...
  paymentForm.value.refund_advance = invoices.value.reduce((sum, inv) => sum + (Number(inv.paying_amount) || 0), 0);
};
...
const handleGlobalAmountInput = () => {
  let remainingAmount = Number(paymentForm.value.refund_advance) || 0;
  if (remainingAmount > grandTotalDue.value) {
    remainingAmount = grandTotalDue.value;
    paymentForm.value.refund_advance = remainingAmount;
...
const submitPayment = async () => {
  ...
  if (paymentForm.value.receiving_advance <= 0 && paymentForm.value.refund_advance <= 0) {
    Swal.fire({ toast: true, position: 'top-end', icon: 'warning', title: 'Please enter a receiving or refund amount', showConfirmButton: false, timer: 3000 });
    return;
  }

  submitting.value = true;
  try {
    await axios.post(`/api/clients/${clientId}/advance`, {
      payment_method: paymentForm.value.payment_method,
      note: paymentForm.value.note,
      date: paymentForm.value.date,
      receiving_advance: paymentForm.value.receiving_advance,
      refund_advance: paymentForm.value.refund_advance,
